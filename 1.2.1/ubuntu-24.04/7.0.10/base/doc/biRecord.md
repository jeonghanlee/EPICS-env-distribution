...
1. Check to see that the appropriate device support module exists. If it
...
2. `readValue()` is called. See ["Input Records"](#input-records) for details.
3. If PACT has been changed to TRUE, the device support read routine has
...
5. Check alarms: This routine checks to see if the new VAL causes the alarm
...
6. Check if monitors should be invoked:
