...
["Device Support Interface"](#device-support-interface) section.
...
3. AOFF is added.
4. If LINR is `NO CONVERSION` the units conversion is finished after the above
steps.
5. If LINR is `LINEAR` or `SLOPE`, the value from step 3 above is multiplied by
ESLO and EOFF is added to complete the units conversion process.
6. Any other value for LINR selects a particular breakpoint table to be used on the
