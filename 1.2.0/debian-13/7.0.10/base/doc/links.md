...
being converted to a double and provided to the expression as `VAL`. Up to 21
...
- args
    ...
    to the inputs `A`, `B`, `C`, ... `U`. Each input argument may be either a
    numeric literal or an embedded JSON link inside `{}` braces. A `pva` link
...
- time
    ...
    An optional string containing a single upper or lower-case letter `A` ... `U`
